...
export const getGuestsStatistics = async () => {
  const authToken = Cookies.get("token");
  try {
    const response = await axios.get(`${APIurl}guests/statistics/data`, {
      headers: {
        authorization: `Bearer ${authToken}`,
      },
    });
...
export const getMeetingsStatistics = async () => {
  const authToken = Cookies.get("token");
  try {
    const response = await axios.get(`${APIurl}meetings/statistics/data`, {
      headers: {
        authorization: `Bearer ${authToken}`,
      },
    });
